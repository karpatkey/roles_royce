...
from tests.utils import fork_reset_state, fork_unlock_account, get_allowance, LOCAL_NODE_PORT
import math
...
def swap_EURe_for_WXDAI_balancer(amount):
    ...
    tx = vault.functions.batchSwap(swap_kind, batch_swap_steps, assets, funds_management, limits, deadline).transact({"from": SAFE_ADDRESS})
    while True:
        try:
