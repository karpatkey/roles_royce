...
def test_integration_exit_2_1(local_node_eth, accounts):
    ...
    balancer_disassembler.send(txns=txn_transactable, private_key=private_key)
    ctrc = w3.eth.contract(address=RETH_WETH_bpt_address,abi=Abis[blockchain].UniversalBPT.abi)
    balan = ctrc.functions.balanceOf(avatar_safe_address).call()
    assert balan == 2000000000
