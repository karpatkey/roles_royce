...
def test_stresstest(local_node_eth, local_node_gc, accounts, monkeypatch):
    ...
    with open(os.path.join(os.path.dirname(__file__), 'test_stresstest_v2.json'), 'r') as f:
        test_data = json.load(f)

    stresstest_tester = stresstest(w3=w3, position=test_data, percentage=PERCENTAGE, max_slippage=MAX_SLIPPAGE, dao=dao.name, blockchain=dao.blockchain)
