...
from defi_protocols.Balancer import (
    ...
    VEBAL_REWARD_TOKENS,
    ABI_VEBAL_FEE_DISTRIBUTOR
)
from decimal import Decimal
from tests.utils import fork_reset_state, fork_unlock_account, LOCAL_NODE_PORT, ETH_FORK_NODE_URL
    ...
    if block >= int(fee_distributor_blocks[-1]):
        ...
        for fee_distributor_block in fee_distributor_blocks:
            if block >= int(fee_distributor_block):
                fee_distributor_contract = w3.eth.contract(address=VEBAL_FEE_DISTRIBUTORS[fee_distributor_block], abi=ABI_VEBAL_FEE_DISTRIBUTOR)
                tx = fee_distributor_contract.functions.claimTokens(wallet, VEBAL_REWARD_TOKENS[vebal_reward_tokens_block]).transact({"from": wallet})
                while True:
                    try:
                ...
                while txn["logs"] != []:
                    for log in txn["logs"]:
                        if log["topics"][0].hex() == "0xff097c7d8b1957a4ff09ef1361b5fb54dcede3941ba836d0beb9d10bec725de6":
