...
from tests.utils import local_node, accounts, get_balance, steal_token, create_simple_safe, local_node_set_block, top_up_address
from roles_royce.toolshed.anti_liquidation.spark import SparkCDP
from decimal import Decimal
from roles_royce import check, send
...
def test_integration(local_node, accounts):
    ...
    amount_to_repay = cdp.get_delta_of_token_to_repay(target_health_factor, ETHAddr.DAI, rate_model=spark.RateModel.VARIABLE)
    ...
    target_health_factor = 5
    amount_to_repay = cdp.get_delta_of_token_to_repay(target_health_factor, ETHAddr.DAI,
                                                           rate_model=spark.RateModel.VARIABLE)
    assert amount_to_repay == 0

def test_something(local_node):
    w3 = local_node
    block = 17837956
    ...
    cdp = SparkCDP(w3, avatar_safe_address, block=block+1)
    assert initial_DAI_balance == 94798898237423007706087

    target_health_factor = 3
    amount_of_DAI_to_repay= cdp.get_delta_of_token_to_repay(target_health_factor, ETHAddr.DAI, rate_model=spark.RateModel.VARIABLE)
    assert amount_of_DAI_to_repay == approx(182246746503552964110971)

    chi = SparkUtils.get_chi(w3, Chain.ETHEREUM, block=block+1)
    ...
    send([redeem_sDAI], role=5, private_key=private_key, roles_mod_address=roles_mod_address,
             blockchain=Chain.ETHEREUM, web3=w3)
    ...
    send([spark.ApproveToken(token=ETHAddr.DAI, amount=delta_in_DAI_balance),
          spark.Repay(token=ETHAddr.DAI, amount=delta_in_DAI_balance, rate_model=spark.RateModel.VARIABLE,
                      avatar=avatar_safe_address)], role=5, private_key=private_key, roles_mod_address=roles_mod_address, blockchain=Chain.ETHEREUM, web3=w3)
