...
def test_approve_method():
    method = aura.ApproveForBooster(token=ETHAddr.LP80GNO20WETH, amount=123)
    assert method.get_args_list() == [ETHAddr.AURABooster, 123]
    assert method.target_address == ETHAddr.LP80GNO20WETH
...
def test_approve_method_with_roles(web3_eth):
    method = aura.ApproveForBooster(token=ETHAddr.LP80GNO20WETH, amount=1000)
    ROLE = 1
