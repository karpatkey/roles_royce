...
from rolling_roles.constants import GnosisChainAddr
...
approve = GenericMethodTransaction(
    ...
    contract_address=GnosisChainAddr.USDT,
...
def test_multi_or_one_one():
    operation, contract_address, data = multi_or_one([approve], blockchain=Chain.GNOSIS)
    assert operation == Operation.CALL
    assert contract_address == GnosisChainAddr.USDT
...
def test_multi_or_one_multi():
    operation, contract_address, data = multi_or_one([approve, add_liquidity], blockchain=Chain.GNOSIS)
    # when building more than one transaction the Multisend contract is used
    assert operation == Operation.DELEGATE_CALL
    assert contract_address == MULTISENDS[Chain.GNOSIS]
