...
def validate_amounts(amount0_desired: float, amount1_desired: float):
    ...
    if amount0_desired > 0:
        if amount1_desired != 0:
            raise ValueError("amount1_desired must be 0 if amount0_desired is greater than 0")
    
    if amount1_desired > 0:
        if amount0_desired != 0:
            raise ValueError("amount0_desired must be 0 if amount1_desired is greater than 0")

def validate_price_percentage_deviation(token_0_min_price_perc_dev: float, token_0_max_price_perc_dev: float):
...
class NFTPosition():
    def __init__(self,
        ...
        try:
          ...
          raise ValueError("NFT does not exist")
...
def set_and_check_desired_amounts(w3: Web3, owner: Address, amount0_desired: float, amount1_desired: float, pool: Pool, tick_lower: int, tick_upper: int, send_eth: bool):
  if amount0_desired > 0:
      amount1_desired = Decimal(amount0_desired * pool.sqrt_price_x96 * 1.0001**(tick_upper/2)*(pool.sqrt_price_x96-1.0001**(tick_lower/2)*(2**96)))/Decimal((2**96) * (1.0001**(tick_upper/2)*(2**96)-pool.sqrt_price_x96))
  elif amount1_desired > 0:
      amount0_desired = Decimal(amount1_desired * (2**96) * (1.0001**(tick_upper/2)*(2**96)-pool.sqrt_price_x96)) / (Decimal(pool.sqrt_price_x96 * 1.0001**(tick_upper/2)*(pool.sqrt_price_x96-1.0001**(tick_lower/2)*(2**96))))
