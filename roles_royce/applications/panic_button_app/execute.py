...
from roles_royce.roles_modifier import (
    update_gas_fees_parameters_and_nonce,
    set_gas_strategy,
    GasStrategies,
)
...
def execute(dao, blockchain, transaction):
    try:
        ...
        if (
            env.MODE == "production"
        ):  # In production environment, send the transaction to the real blockchain
...
def main():
    ...
    parser.add_argument(
        "-t", "--transaction", type=str, help="Transaction json to execute."
    )
    ...
    response = execute(
        dao=args.dao, blockchain=args.blockchain, transaction=transaction
    )
