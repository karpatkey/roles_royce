...
from defabipedia.tokens import erc20_contract, Addresses
...
class DynamicDataManager:
    ...
    def get_data(self, amount_x3CRV: float, amount_GBPe: float) -> DynamicData:
        ...
        x3CRV_balance = erc20_contract(self.w3, Addresses[Chain.GNOSIS].x3CRV).functions.balanceOf(self.static_data.env.AVATAR_SAFE_ADDRESS).call()
        GBPe_balance = erc20_contract(self.w3, Addresses[Chain.GNOSIS].GBPe).functions.balanceOf(self.static_data.env.AVATAR_SAFE_ADDRESS).call()
        bot_xDAI_balance = self.w3.eth.get_balance(self.static_data.env.BOT_ADDRESS)
        return DynamicData(amount_x3CRV=amount_x3CRV,
...
class Swapper:
    ...
    def swap_GBPe_for_x3CRV(self, static_data: StaticData, dynamic_data: DynamicData) -> TxReceipt:
        min_amount_out = int(
            Decimal(1 - self.max_slippage) * Decimal(dynamic_data.GBPe_to_x3CRV) * Decimal(10 ** static_data.decimals_x3CRV))
        amount = int(Decimal(dynamic_data.amount_GBPe) * Decimal(10 ** static_data.decimals_GBPe))
        return roles.send(
            [SwapGBPeForx3CRV(amount=amount, min_amount_out=min_amount_out,
    ...
    def swap_x3CRV_for_GBPe(self, static_data: StaticData, dynamic_data: DynamicData) -> TxReceipt:
        min_amount_out = int(
            Decimal(1 - self.max_slippage) * Decimal(dynamic_data.x3CRV_to_GBPe) * Decimal(10 ** static_data.decimals_GBPe))
        amount = int(Decimal(dynamic_data.amount_x3CRV) * Decimal(10 ** static_data.decimals_x3CRV))
        return roles.send(
            [Swapx3CRVforGBPe(amount=amount, min_amount_out=min_amount_out,
                              avatar=self.avatar_safe_address)],
            role=self.role, private_key=self.private_keys, roles_mod_address=self.roles_mod_address, web3=self.w3)
