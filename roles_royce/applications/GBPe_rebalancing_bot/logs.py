from roles_royce.applications.GBPe_rebalancing_bot.core import StaticData, DynamicData
...
def log_initial_data(static_data: StaticData, messenger: Messenger):
    ...
    message = (f"  Avatar safe address: {static_data.env.AVATAR_SAFE_ADDRESS}\n"
...
def log_status_update(static_data: StaticData, dynamic_data: DynamicData):
    ...
    message = (
        ...
        f'\n'
        f'  USD to GBP oracle: {dynamic_data.amount_x3CRV:,.3f} USD ---> {dynamic_data.amount_x3CRV * dynamic_data.x3CRV_price / dynamic_data.GBP_price:,.3f} GBPe.\n'
