...
def bot_do(w3_eth, w3_gnosis, static_data: StaticData) -> int:
    ...
    if 10 * amount_x3CRV * (10 ** decimals_x3CRV) < balance_x3CRV and amount_x3CRV < env.AMOUNT:
        ...
        if flags.lack_of_x3CRV.is_set() is False:
            ...
            messenger.log_and_alert(LoggingLevel.Warning, title, message,
                                    alert_flag=flags.lack_of_x3CRV.is_set())
    ...
    if 10 * amount_GBPe * (10 ** decimals_GBPe) < balance_GBPe and amount_GBPe < env.AMOUNT:
        ...
        if flags.lack_of_GBPe.is_set() is False:
            ...
            messenger.log_and_alert(LoggingLevel.Warning, title, message,
                                    alert_flag=flags.lack_of_GBPe.is_set())
    ...
    swapper = Swapper(w3=w3_gnosis, private_keys=env.PRIVATE_KEY, role=env.ROLE, roles_mod_address=env.ROLES_MOD_ADDRESS,
    ...
    if dynamic_data.drift_GBPe_to_x3CRV > env.DRIFT_THRESHOLD and balance_GBPe >= amount_GBPe * (10 ** decimals_GBPe):
        ...
        message, message_slack = get_tx_receipt_message_with_transfers(tx_receipt_GBPe_to_x3CRV,
                                                                       env.AVATAR_SAFE_ADDRESS, w3_gnosis)
        messenger.log_and_alert(LoggingLevel.Info, f'{amount_GBPe:,.3f} GBPe swapped for x3CRV', message,
        ...
        message, message_slack = get_tx_receipt_message_with_transfers(tx_receipt_x3CRV_to_GBPe,
                                                                       env.AVATAR_SAFE_ADDRESS, w3_gnosis)
        messenger.log_and_alert(LoggingLevel.Info, f'{amount_x3CRV:,.3f} x3CRV swapped for GBPe', message,
                                slack_msg=message_slack)
