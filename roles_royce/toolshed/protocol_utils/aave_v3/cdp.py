...
class AaveV3CDPManager:
    ...
    def get_cdp_balances_data(self, block: int | str = 'latest') -> list[dict]:
        ...
        for element in aave_v3_tokens:
            ...
            # Wallet balances
            wallet_data = protocol_data_provider_contract.functions.getUserReserveData(element[AaveV3Token.UNDERLYING],
            ...
            liquidation_threshold = \
                protocol_data_provider_contract.functions.getReserveConfigurationData(
                    element[AaveV3Token.UNDERLYING]).call(block_identifier=block)[
                    2] / Decimal(10_000)
    ...
    def get_cdp_data(self, block: int | str = 'latest') -> AaveV3CDP:
        ...
        return AaveV3CDP(owner_address=self.owner_address, blockchain=self.blockchain, block=block,
                        balances_data=balances_data, health_factor=health_factor)

    def check_if_token_is_in_debts(self, aave_v3_cdp: AaveV3CDP, token_address: Address | ChecksumAddress | str) -> dict:
    ...
    def get_delta_of_token_to_repay(self, aave_v3_cdp: AaveV3CDP, target_health_factor: float | Decimal,
                                    token_in_address: str,
                                    rate_model: InterestRateMode, block: int | str = 'latest', tolerance: float = 0.01) -> int:
        ...
        token_in_borrowed_status = self.check_if_token_is_in_debts(aave_v3_cdp=aave_v3_cdp, token_address=token_in_address)

        if token_in_borrowed_status[InterestRateMode.STABLE] is False and token_in_borrowed_status[
            InterestRateMode.VARIABLE] is False:
    ...
    def repay_single_token_debt(self, aave_v3_cdp: AaveV3CDP, token_in_address: str | ChecksumAddress | Address,
