...
Artemis provides a way to declare protocol functions, called :class:`~artemis.protocols.base.ContractMethod`,
and some functions to interact with the Roles Modifier contract:

- :meth:`artemis.roles.check` to statically check if a transaction will go through or will fail.
- :meth:`artemis.roles.send` to actually send the transaction to the blockchain.
- :meth:`artemis.roles.build` to create a transaction to later be sent to the blockchain
...
``artemis/protocols/eth/myproto.py``.
In that file create a subclass of :class:`~artemis.protocols.base.ContractMethod` per function
...
.. code-block:: python
    ...
    from artemis.protocols.base import ContractMethod, Address
...
.. code-block:: python
    ...
    from artemis.utils import simulate_tx
